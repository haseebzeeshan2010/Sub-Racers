...
func _physics_process(delta):
	...
	if camera:
		...
		# Define a plane at the sub's Y position (or ground level)
		...
		# Fallback: If horizontal plane intersection failed, use a vertical plane ahead of the sub
	...
	if use_keyboard_steering:
		...
		# Create a target position at the same y-level as the sub
		...
		# Get sub's current forward direction
